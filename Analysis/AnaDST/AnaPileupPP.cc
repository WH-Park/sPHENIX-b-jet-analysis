...
//----------------------------------------------------------------------------//
AnaPileupPP::AnaPileupPP(const string &name):
  SubsysReco( name ),
  _events( NULL )
{
  //initialize
  _event = 0;
  _outfile = "AnaPileupPP.root";
...
//----------------------------------------------------------------------------//
int AnaPileupPP::Init(PHCompositeNode *topNode)
{
  ...
  _events->Branch("event",0,"event/I");
	...
	_events->Branch("gen_vt",0,"gen_vt[npart]/F");
	...
	_events->Branch("g4hit_x",0,"g4hit_x[ng4hit]/F");
	_events->Branch("g4hit_y",0,"g4hit_y[ng4hit]/F");
	_events->Branch("g4hit_z",0,"g4hit_z[ng4hit]/F");

	_events->Branch("ncluster",0,"ncluster/I");
	_events->Branch("cluster_layer",0,"cluster_layer[ncluster]/S");
	_events->Branch("cluster_x",0,"cluster_x[ncluster]/F");
	_events->Branch("cluster_y",0,"cluster_y[ncluster]/F");
	_events->Branch("cluster_z",0,"cluster_z[ncluster]/F");
	...
	_events->Branch("track_cluster_n_maps_embed",0,"track_cluster_n_maps_embed[ntrack]/S");
	_events->Branch("track_cluster_n_intt_embed",0,"track_cluster_n_intt_embed[ntrack]/S");
	_events->Branch("track_cluster_n_tpc_embed",0,"track_cluster_n_tpc_embed[ntrack]/S");
	...
	_events->Branch("track_gvt",0,"track_gvt[ntrack]/F");
	...
	hz_out_trk_tpc = new TH1D("hz_out_trk_tpc","",200,-100,100);
	hz_out_trk_tpc_mvtx = new TH1D("hz_out_trk_tpc_mvtx","",200,-100,100);
...
//----------------------------------------------------------------------------//
int AnaPileupPP::process_event(PHCompositeNode *topNode)
{
  _event++;
  //if (1)
  if (_event % 1000 == 0)
...
//----------------------------------------------------------------------------//
int AnaPileupPP::End(PHCompositeNode *topNode)
{

	cout << "-----AnaPileupPP::End------" << endl;
  ...
  _events->Write();
	...
	hz_out_trk_tpc->Write();
	hz_out_trk_tpc_mvtx->Write();
...
//----------------------------------------------------------------------------//
void AnaPileupPP::fill_tree(PHCompositeNode *topNode)
{
  ...
  reset_variables();
	...
	*/
	...
	//if ( 0 ){
		...
		for (PHG4TruthInfoContainer::ConstIterator iter=range.first; iter!=range.second; ++iter)
		{
			...
			if ( embedding_id==2 ){
				...
				if ( fabs(vec.Eta())<1.0 && abs(pid)==211 ){
				...
				if ( vtx ){
					...
					if ( ntpc_layer>=35 ){
						hz_out_trk_tpc->Fill(vtx->get_z());
						if (nmaps_layer>=2 ){
							hz_out_trk_tpc_mvtx->Fill(vtx->get_z());
						}
					}
	...
	if ( 0 ){
		...
		PHG4HitContainer::ConstRange range =  _g4hit_container->getHits();
		...
		for (PHG4HitContainer::ConstIterator iter=range.first; iter!=range.second; ++iter)
		{
			...
			g4hit_x[ng4hit] = g4hit->get_avg_x();
			g4hit_y[ng4hit] = g4hit->get_avg_y();
			g4hit_z[ng4hit] = g4hit->get_avg_z();
	...
	if ( 0 ){
		...
		for (SvtxClusterMap::ConstIter iter=_clustermap->begin(); iter!=_clustermap->end(); ++iter){
			...
			int layer = svtx_cluster->get_layer();
			if ( layer<_n_maps_layers || layer>=(_n_maps_layers+_n_intt_layers) ) continue;

			cluster_layer[ncluster] = short(layer);
			cluster_x[ncluster] = svtx_cluster->get_x();
			cluster_y[ncluster] = svtx_cluster->get_y();
			cluster_z[ncluster] = svtx_cluster->get_z();
	...
	//if ( 0 ){
		for (SvtxTrackMap::ConstIter iter=_trkmap->begin(); iter!=_trkmap->end(); ++iter){
			...
			if ( g4particle ){
				...
				track_cluster_n_maps_truth[ntrack] =  trackeval->get_layer_range_contribution(svtx_trk,g4particle,0,_n_maps_layers);
				track_cluster_n_intt_truth[ntrack] = trackeval->get_layer_range_contribution(svtx_trk,g4particle,_n_maps_layers,_n_maps_layers+_n_intt_layers);
				track_cluster_n_tpc_truth[ntrack] = trackeval->get_layer_range_contribution(svtx_trk,g4particle,_n_maps_layers+_n_intt_layers,_n_maps_layers+_n_intt_layers+_n_tpc_layers);
			}//g4particle
			...
			*/
			...
			track_cluster_n_maps_embed[ntrack] = nmaps_truth;
			track_cluster_n_intt_embed[ntrack] = nintt_truth;
			track_cluster_n_tpc_embed[ntrack] = ntpc_truth;
	...
	*/
	...
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(gen_vt);
	...
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(g4hit_x);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(g4hit_y);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(g4hit_z);
	...
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(cluster_x);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(cluster_y);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(cluster_z);
	...
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(track_cluster_n_maps_embed);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(track_cluster_n_intt_embed);
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(track_cluster_n_tpc_embed);
	...
	((TBranch*) _events->GetListOfBranches()->At(count++))->SetAddress(track_gvt);
...
//----------------------------------------------------------------------------//
void AnaPileupPP::reset_variables()
{
	...
	for (int ic=0; ic<2000; ic++){
		cluster_layer[ic] = g4hit_layer[ic] = -1;
		cluster_x[ic] = cluster_y[ic] = cluster_z[ic] = -9999;
		g4hit_x[ic] = g4hit_y[ic] = g4hit_z[ic] = -9999;
	}
	...
	*/
	...
	for (int it=0; it<20000; it++){
		...
		gen_vx[it] = gen_vy[it] = gen_vz[it] = gen_vt[it] = -9999;
	...
	for (int it=0; it<2000; it++){
		...
		track_cluster_n_maps_embed[it] = track_cluster_n_intt_embed[it] = track_cluster_n_tpc_embed[it] = 0;
		...
		track_gvx[it] = track_gvy[it] = track_gvz[it] = track_gvt[it] = -999;
...
//----------------------------------------------------------------------------//
void AnaPileupPP::GetNodes(PHCompositeNode * topNode)
{
  //DST objects
	//
  ...
  //PHG4HitContainer
  _g4hit_container = findNode::getClass<PHG4HitContainer>(topNode,"G4HIT_INTT");
  if (!_g4hit_container && _event<2)
  {
    cout << PHWHERE << " G4HIT_INTT node not found on node tree" << endl;
  }
